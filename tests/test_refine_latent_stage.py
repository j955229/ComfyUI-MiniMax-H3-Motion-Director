...
def test_sync_keyframes_removes_motion_context_for_existing_repin(monkeypatch):
    import nodes

    class Decode:
        def decode(self, vae, latent):
            raise AssertionError("Removed Motion Context keyframes must not be re-encoded")
    ...
    out = sync_h3_keyframe_conditioning(conditioning, FakeVAE(), width=128, height=64)
    assert out[0][1]["minimax_keyframes"] == []
