...
def build_rect_masks(transform: dict[str, Any], config: dict[str, Any], device, dtype) -> torch.Tensor:
    ...
    masks = torch.zeros((count, 1, canvas_h, canvas_w), device=device, dtype=dtype)
    paste_region = config.get("paste_region") or "face_rect"
    mode = config.get("mask_mode") or "rect"
    dilation_ratio = float(config.get("mask_dilation") or 0.0)
    for index in range(count):
        ...
        if mode == "ellipse":
            ...
            masks[index, 0] = ((((xx - (x + width / 2)) / rx) ** 2 + ((yy - (y + height / 2)) / ry) ** 2) <= 1).to(dtype)
        else:
    ...
    radius = round(feather * min(canvas_w, canvas_h))
    return _gaussian_blur(masks.float(), radius).clamp(0, 1).to(dtype)


def build_sam_masks(crops: torch.Tensor, transform: dict[str, Any], config: dict[str, Any]) -> torch.Tensor:
    """Load SAM only when explicitly selected; any failure aborts Face Refine."""
    model_name = str(config.get("sam_model") or "")
    if not model_name:
        raise ValueError("SAM mask mode was selected but no internal SAM model was selected.")
    import folder_paths
    try:
    ...
    for category in ("sams", "sam", "ultralytics_segm"):
        ...
        if path:
            break
    if not path:
        raise FileNotFoundError(f"SAM model not found: {model_name}")
    model = SAM(path)
    masks = []
    threshold = float(config.get("sam_threshold") or 0.5)
    ...
    dilation = round(float(config.get("sam_dilation") or 0.0) * min(stack.shape[-2:]))
    if dilation > 0:
        stack = F.max_pool2d(stack, dilation * 2 + 1, stride=1, padding=dilation)
    ...
    feather = round(float(config.get("feather") or 0.0) * min(stack.shape[-2:]))
    return _gaussian_blur(stack, feather).clamp(0, 1)
